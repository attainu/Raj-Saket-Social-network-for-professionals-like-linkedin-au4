...
router.post('/',
 ...
 async (req, res) => {
   ...
   //console.log('details is here->', req.body);
    ...
    if (!errors.isEmpty()) {
     ...
     try {
       ...
       const payload = {                 //getting the paylaod which includes user _id
         user: {
           id:  user.id
         }
       };
       ...
       jwt.sign(                          //sign the token                        
         ...
         {expiresIn: 360000},
         (error,token) => {               //here in callback we will get either error or TOKEN. Afetr we get token we will send it back to the client.
